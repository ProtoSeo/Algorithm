...
int main(){
    ...
    int i,j,n,max = 1;
    cin >>n;
    for(i=1;i<=n;i++) cin >> a[i].first >> a[i].second; //입력
    sort(a,a+n+1);  //첫번째 전봇대를 기준으로 정렬을 한다.
    d[1] = 1;   
    for(i=2;i<=n;i++){  //LIS O(n^2) 알고리즘  
        d[i]=1;
        for(j=1;j<=i;j++){
            if(a[i].second > a[j].second && d[i]<d[j]+1){   //첫번째 전봇대를 기준으로 정렬했으므로 두번째 전봇대의 값으로 LIS를 찾는다.
                d[i] = d[j]+1;
            }
        }
        if(max<d[i])
