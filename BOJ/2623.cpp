...
둘째 줄부터 각 보조 PD가 정한 순서들이 한 줄에 하나씩 나온다. 각 줄의 맨 앞에는 보조 PD가 담당한 가수의 수가 나오고, 그 뒤로는 그 가수들의 순서가 나온다. N은 1이상 1,000이하의 정수이고, M은 1이상 100이하의 정수이다.
...
int main(){
    ...
    vector<int> res;
    set<pair<int,int>> edge;    //set을 이용한 이유 : 한명의 가수를 여러명의 PD가 순서를 정할수 있으므로 중복되는 것을 막기 위해서 set을 사용함
    queue<int> q;
    for(i=0;i<m;i++){   //입력을 받는 반복문
        int tmp;
        cin >>tmp;
        vector<int> input(tmp);
        for(j=0;j<tmp;j++){
            cin >> input[j];
            if(j!=0){      //set에 넣는 조건문 (j=1일때부터 한 쌍씩 넣는다.)
                edge.insert({input[j-1],input[j]});
            }
        }
    }
    set<pair<int,int>>::iterator it;    //set의 iterator
    for (it = edge.begin(); it != edge.end(); ++it){    //이제 set을 통해서 중복되는 입력은 없을테니 graph와 선행자 배열을 만든다.
        pair<int,int> p = *it;
        graph[p.first].push_back(p.second);
        pre[p.second]++;
    }
    for(i=1;i<n+1;i++){     //선행자가 0인 정점을 queue에 푸쉬한다.
        if(pre[i]==0){
            q.push(i);
        }
    }
    while(!q.empty()){      //위상정렬
        int v = q.front();
        q.pop();
        res.push_back(v);   //위상정렬을 통해서 순서가 정해진 값을 res vector에 저장하도록 한다.
    ...
    }else{  //위상정렬의 결과값과 n의 값이 다르면 사이클이 존재해서 위상정렬이 되지않은것이다. 따라서 순서를 정할 수 없으므로 0을 출력.
        cout << 0;
    }
}