...
int main(){
    ...
    for(i=0;i<m;i++){   // 정점의 위치를 입력받아서 단방향성 인접리스트를 생성한다.
        ...
        cin >> a>>b;
        graph[a].push_back(b);  
        predecessor[b]++;   //동시에 향해지는 정점의 위치의 선행자의 개수를 증가시킨다.
    }
    ...
    while(!q.empty()){      //선행자를 담는 우선순위 큐가 비어질 때 까지 반복문을 반복
        int v = q.top();
        cout << v << ' ';
        q.pop();
        for (int nextV : graph[v]) {    //그리고 해당 정점에서 연결이 되어있는 다른 정점이  있는지 확인하는 반복문
            predecessor[nextV]--;   // 연결이 되어있다면 해당 정점의 선행자의 수를 감소시킴
            if (predecessor[nextV] == 0) {      //그리고 선행자가 0이라면 큐에 푸쉬해준다.
                q.push(nextV);
            }
        }
    }
