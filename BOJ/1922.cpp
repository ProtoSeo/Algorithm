...
int main(){
    ...
    int i,n,m,ans=0;
    cin>> n>>m; 
    for(i=0;i<m;i++){   //크루스칼 알고리즘을 위해서 edge를 저장한다.
        int s,e,v;
        cin >>s>>e>>v;
        edge.push_back({v,{s,e}});
    }
    //최소비용인 선을 꺼내서 최소 스패닝 트리를 만들어야 하므로 비용을 기준으로 오름차순으로 정렬한다.
    sort(edge.begin(),edge.end());  
    //크루스칼 알고리즘 구현 부분
    for(auto e : edge){ 
        pair<int,int> loc = e.second;
        int x = loc.first;
        int y = loc.second;
         //같은 disjoint set이 아니라면 병합하고 해당 비용만큼 최소스패닝 트리의 비용을 증가시킨다.
        if(find(x)!=find(y)){  
            merge(x,y);
            ans+=e.first;
        }
