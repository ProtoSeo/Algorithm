...
int main(){
    ...
    while(t--){
        int n,w,k,i;
        cin >>n>>k;
        vector<int> d(n+1,0);     //각 건물의 건설시간을 저장하는 vector
        vector<int> res(n+1,0);     //정답의 건설시간을 구하기 위한 vector
        vector<int> pre(n+1,0);     //선행자를 저장하기 위한 vector
        vector<int> graph[1001];    //단방향성 인접리스트
        queue<int> q;   // 위상정렬을 하기위해서 선행자가 0인 정점을 저장하는 queue
        for(i=1;i<n+1;i++) cin >> d[i];
        for(i=0;i<k;i++){   //단방향성 인접리스트를 만들고, 선행자를 저장하는 vector를 만든다.
            int a,b;
            cin >> a >> b;
            graph[a].push_back(b);
            pre[b]++;
        ...
        cin >> w;
        for(i=1;i<n+1;i++){     // 선행자가 0인 정점을 저장하는 반복문  
            if(pre[i]==0){
                q.push(i);
                res[i] = d[i];  // 선행자가 0이면 res[i]의 값은 입력받은 건설시간과 같다. 
            }
        }
        while(!q.empty()){  //위상정렬을 실행하는 반복문
            int v = q.front();  //저장되어있는 q에서 정점을 하나 뽑은뒤에
            q.pop();
            for(int nextV : graph[v]){  //해당 정점에서의 가리키는 정점들의 선행자를 1씩 감소시켜주는 반복문
                pre[nextV]--;
                if(pre[nextV] == 0){    //그리고 선행자가 0이라면 queue에 담는다.
                    q.push(nextV);
                }
                res[nextV] = max(res[nextV],res[v]+d[nextV]);   //res배열에서 현재 값과 이전정점에서 넘어오는 값을 비교해서 큰값을 저장하도록 한다.  
            }
        }
        cout << res[w] << '\n';
    }
